...
column_name_mapping = open(f"{MAPPING_DIR}/ods_club_mapping.json", 'rb').read()
column_name_mapping = json.loads(column_name_mapping)
 
def run():
    
    ODS_Club.objects.all().delete()
    ...
    mask = data['region'] == "Auvergne-Rhône-Alpes"
    data = data[mask]
    mask = data['code_qpv'] != "NR - Non réparti"
    data = data[mask]
